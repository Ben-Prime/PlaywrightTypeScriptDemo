...
- Name: Test_Talk_Tutorial_Ch_2\assertions_test.spec.ts >> Soft Assertions in playwright
- Location: tests\Test_Talk_Tutorial_Ch_2\assertions_test.spec.ts:31:5
...
Call log:
  - Expect "soft toBeHidden" with timeout 30000ms
  - waiting for getByPlaceholder('Search', { exact: true }).first()
    33 × locator resolved to <input type="text" role="combobox" autocorrect="off" aria-controls="i0" autocomplete="off" spellcheck="false" name="search_query" aria-expanded="true" placeholder="Search" aria-autocomplete="list" class="ytSearchboxComponentInput yt-searchbox-input title"/>
      ...
      - generic [ref=e147]:
        ...
        - generic [ref=e194]:
          ...
          - link "More options" [ref=e213] [cursor=pointer]:
            - /url: https://consent.youtube.com/d?continue=https://www.youtube.com/%3Fcbrd%3D1&gl=NG&m=0&pc=yt&cm=2&hl=en&src=2&escs=AZ8E49CnEz5ILersriL7fz5iT1vSv-20U8jOrKYDlD_faqviu5xkGuRRwtCKTTAoW8Y9QfRed2JrIkoD5uuArV1jpyfvgQDSZQdw
...
```ts
  ...
  2  | 
  3  | test('Hard Assertions in playwright', async ({ page }) => {
  4  |   test.slow();
  5  |   // Navigate to the url and handle cookie consent if present
  6  |   await page.goto('https://youtube.com/');
  7  | 
  8  |   const acceptButton = page.locator('button:has-text("Accept all")').first();
  9  |   if (await acceptButton.isVisible().catch(() => false)) {
  10 |     await acceptButton.click();
  11 |     await page.waitForLoadState('domcontentloaded');
  12 |   }
  13 | 
  14 |   const searchInput = page.getByPlaceholder('Search', { exact: true }).first();
  15 |   // Visible, editable, enabled, empty
  16 |   await expect(searchInput).toBeVisible();
  17 |   await expect(searchInput).toBeEditable();
  18 |   await expect(searchInput).toBeEnabled();
  19 |   await expect(searchInput).toBeEmpty();
  20 | 
  21 |   // Fill the search input and assert URL, title, text, count
  22 |   await searchInput.click();
  23 |   await searchInput.fill('playwright by testers talk');
  24 |   await searchInput.press('Enter');
  25 |   await expect(page).toHaveURL(/results\?search_query=playwright\+by\+testers\+talk/);
  26 |   await expect(page).toHaveTitle(/playwright by testers talk - YouTube/);
  27 |   await expect(page.locator('[id="text"]').nth(2)).toHaveText('Testers Talk'); // Assert that the third result contains "Testers Talk"
  28 |   await expect(page.getByPlaceholder('Search')).toHaveCount(1); // Assert that there are 20 results on the page
  29 | });
  30 | 
  31 | test('Soft Assertions in playwright', async ({ page }) => {
  32 |   test.slow();
  33 | 
  34 |   await page.goto('https://youtube.com/');
  35 | 
  36 |   const acceptButton = page.getByRole('button', { name: 'Accept all' }).first();
  37 |   if (await acceptButton.isVisible().catch(() => false)) {
  38 |     await acceptButton.click();
  39 |     await page.waitForLoadState('domcontentloaded');
  40 |   }
  41 |   const searchInput = page.getByPlaceholder('Search', { exact: true }).first();
> 42 |   await expect.soft(searchInput).toBeHidden();
     |                                  ^ Error: expect(locator).toBeHidden() failed
  43 |   await expect(searchInput).toBeEditable();
  44 | });
```